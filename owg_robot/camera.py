...
class Camera:
    ...
    def get_cam_img(self):
        ...
        # convert rgb from a flat array to shape (height, width, 4)
        rgb = np.reshape(rgb, (self.height, self.width, 4))[:, :, :3].astype(np.uint8)
        depth = np.reshape(depth, (self.height, self.width))
        seg = np.reshape(seg, (self.height, self.width))
